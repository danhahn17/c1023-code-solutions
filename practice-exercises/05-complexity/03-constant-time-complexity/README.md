...
```js
function accessElement(arr: unknown[], index: number): number {
  return arr[index];
}
```
...
We can test this in VS Code by adding the following to a .ts file:

```js
function accessElement(arr: unknown[], index: number): unknown {
  return arr[index];
}
