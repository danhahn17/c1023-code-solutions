...
describe('BinarySearchTree', () => {
  ...
  test('should return undefined for non-existing nodes using lookup', () => {
    bst.insert(10);
    bst.insert(5);
    bst.insert(15);

    expect(bst.lookup(2)).toBeUndefined();
    expect(bst.lookup(8)).toBeUndefined();
    expect(bst.lookup(20)).toBeUndefined();
  });
  ...
  test('should remove nodes correctly', () => {
    ...
    bst.remove(15);
    expect(bst.lookup(15)).toBeUndefined();

    bst.remove(10);
    expect(bst.lookup(10)).toBeUndefined();

    bst.remove(7);
    expect(bst.lookup(7)).toBeUndefined();
  });
  ...
  test('should print tree in-order', () => {
    const consoleSpy = jest
      .spyOn(console, 'log')
      .mockImplementation(() => undefined); // Spy on console.log and mock the implementation
